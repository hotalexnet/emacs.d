...
这是我的个人 Emacs 配置，主要面向 Arch Linux / GNU Linux 桌面环境。主配置在 `init.el`，启动优化在 `early-init.el`，本机私有信息放在不会提交的 `local.el`。
...
├── dict/                # 本地词典文件
├── emacs-chess-cn/      # 中国象棋扩展，以 submodule 方式跟踪
...
这份配置优先使用 Emacs 内置的 tree-sitter major modes，包括：
...
也可以按你的 Python 环境自行安装，只要保证 `~/.local/bin/aider` 可执行。
